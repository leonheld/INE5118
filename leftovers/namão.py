...
#usei regex do vim pra criar esse vetor
#total
...
#tirar vários valores de 0 até 10, que não fazem realmente parte dos stats dos pokemons e só servem pra pokemons de base
...
#plt.hist(organized_total.keys(), bins=10)

sns.distplot(total, kde=True, norm_hist=1)
...
#pmf = np.random.binomial(len(total), 0.01, 32)
#plt.hist(pmf, bins = 20)
#sns.distplot(pmf, kde=True)

#texto e tudo mais
plt.xlabel(r'HP de Pokemons')
plt.ylabel(r'frequência relativa')
plt.title(r"Frequência relativa de HP de Pokemons")
plt.show()